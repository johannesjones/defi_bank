...
contract('dBank', ([deployer, user]) => {
  ...
  describe('testing token contract...', () => {
    describe('success', () => {
      ...
      it('checks token initial total supply', async () => {
        expect(Number(await token.totalSupply())).to.eq(0)
      })

      it('makes sure dBank should have Token minter role', async () => {
        expect(await token.minter()).to.eq(dbank.address)
      })
    })

    describe('failure', () => {
      it('checks passing minter role should be rejected', async () => {
        await token.passMinterRole(user, {from: deployer}).should.be.rejectedWith(EVM_REVERT)
      })

      it('checks tokens minting should be rejected', async () => {
  ...
  describe('testing deposit...', () => {
    ...
    describe('success', () => {
      ...
      it('checks balance should increase', async () => {
        expect(Number(await dbank.etherBalanceOf(user))).to.eq(10**16)
      })

      it('checks that deposit time should > 0', async () => {
        expect(Number(await dbank.depositStart(user))).to.be.above(0)
      })

      it('checks that deposit status should eq true', async () => {
        expect(await dbank.isDeposited(user)).to.eq(true)
      })
    })

    describe('failure', () => {
      it('checks that depositing should be rejected', async () => {
  ...
  describe('testing withdraw...', () => {
    ...
    describe('success', () => {
      ...
      it('checks that depositer data should be reseted', async () => {
    ...
    describe('failure', () => {
      it('checks that withdrawing should be rejected', async () =>{
        await dbank.deposit({value: 10**16, from: user}) //0.01 ETH
        await wait(2) //accruing interest
  ...
  describe("testing payOff...", () => {
      describe("success", () => {
          beforeEach(async () => {
              await dbank.borrow({ value: 10 ** 16, from: user }); //0.01 ETH
              await token.approve(dbank.address, (5 * 10 ** 15).toString(), {
                  from: user,
              });
